...
class CompleteTaskUseCase(GetTaskDependantTaskUseCase):
    async def execute(self, invoker_id: UUID, task_id: UUID) -> Task:
        invoker_member, task = await self.get_task.execute(
            invoker_id=invoker_id, task_id=task_id
        )
        task.complete(invoker_id, override=invoker_member.can_override_task)
